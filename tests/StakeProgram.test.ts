...
describe('StakeProgram', () => {
  describe('initialization', () => {
    it('should initialize with SDK', () => {
      const sdk = baseSdk();
      const stake = new StakeProgram(sdk);

      expect(stake).toBeDefined();
      expect((stake as any).sdk).toBe(sdk);
      expect(stake.client).toBe(stakingClient);
    });
    ...
    it('should have access to program ID from config', () => {
      ...
      const programId = (stake as any).getProgramId();
      expect(programId).toBe(sdk.config.programs.stakeAddress);
    });
  });
  ...
  describe('transforms', () => {
    ...
    beforeEach(() => {
      stake = new StakeProgram(baseSdk());
    });

    it('transformStakeAccount converts bigint to numbers and includes address', () => {
      const acc = makeStakeAccount(5000, newAddr(100));
      const out = stake.transformStakeAccount(acc);
    ...
    it('transformStakeAccount handles all stake account fields', () => {
    ...
    it('transformStakeAccount excludes discriminator', () => {
      const acc = makeStakeAccount(1000);
      const out = stake.transformStakeAccount(acc);
  ...
  describe('integration scenarios', () => {
    ...
    it('should handle large amounts with xnos', async () => {
      ...
      const transformed = stake.transformStakeAccount(largeStake);
