...
import { NosanaClient, NosanaNetwork, NosService } from '../src/index.js';
import { address } from 'gill';

// Mock gill's createSolanaClient
...
describe('NosService', () => {
  ...
  describe('initialization', () => {
    it('should initialize NosService', () => {
      expect(nosService).toBeInstanceOf(NosService);
    });

    it('should have access to NosanaClient instance', () => {
      expect((nosService as any).sdk).toBe(client);
    });
  ...
  describe('network-specific token mints', () => {
    it('should use mainnet NOS token address on mainnet', () => {
      const mainnetClient = new NosanaClient(NosanaNetwork.MAINNET);
      const mainnetNosService = mainnetClient.nos;

      expect((mainnetNosService as any).getNosTokenMint()).toBe(
        address('nosXBVoaCTtYdLvKY6Csb4AC8JCdQKKAaWYtx2ZMoo7')
      );
    });

    it('should use devnet NOS token address on devnet', () => {
      const devnetClient = new NosanaClient(NosanaNetwork.DEVNET);
      const devnetNosService = devnetClient.nos;

      expect((devnetNosService as any).getNosTokenMint()).toBe(
