...
export async function complete({ job, ipfsResultsHash }: CompleteParams, { client, get, getRequiredWallet }: InstructionsHelperParams): Promise<CompleteInstruction> {
  const wallet = getRequiredWallet();
  const { state } = await get(job);

  if (state !== JobState.STOPPED) throw new Error(`Cannot complete a job that has not been stopped. Current state: ${JobState[state]}`);
