  ...
  describe('vaults', () => {
    describe('create', () => {
      test('when signer auth is present, it should create a vault', async () => {
        (global.TEST_ROUTE_OPTIONS_WITH_SIGNER.client.POST as Mock).mockResolvedValue({
          data: global.TEST_MOCK_VAULT,
          error: null,
        });

        const api = createDeploymentsApi(global.TEST_ROUTE_OPTIONS_WITH_SIGNER, false);
        const result = await api.vaults.create();

        expect(result.address).toBe(global.TEST_MOCK_VAULT.vault);
        expect(result.created_at).toBeInstanceOf(Date);
        expect(global.TEST_ROUTE_OPTIONS_WITH_SIGNER.client.POST).toHaveBeenCalledWith(
      ...
      test('when solana not in options, it should throw error', async () => {
        const api = createDeploymentsApi({ client: global.TEST_MOCK_CLIENT }, false);
    ...
    describe('list', () => {
      test('when signer auth is present, it should list vaults', async () => {
        (global.TEST_ROUTE_OPTIONS_WITH_SIGNER.client.GET as Mock).mockResolvedValue({
        ...
        expect(global.TEST_ROUTE_OPTIONS_WITH_SIGNER.client.GET).toHaveBeenCalledWith(
