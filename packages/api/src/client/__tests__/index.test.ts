...
import { createNosanaClient } from '../index.js';
...
describe('createNosanaClient', () => {
  test('when called with custom backend_url option, it should use the custom URL', () => {
    createNosanaClient(NosanaNetwork.MAINNET, global.TEST_API_KEY, { backend_url: 'https://custom.api.com' });
  ...
  test('when called without options, it should use the default mainnet URL', () => {
    createNosanaClient(NosanaNetwork.MAINNET, global.TEST_API_KEY, undefined);
  ...
  test('when called with devnet environment, it should use the devnet URL', () => {
    createNosanaClient(NosanaNetwork.DEVNET, global.TEST_API_KEY, undefined);
  ...
  test('when using SignerAuth, it should set x-user-id and Authorization headers', async () => {
    createNosanaClient(NosanaNetwork.MAINNET, testSignerAuth, undefined);
